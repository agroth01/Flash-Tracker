...
# struct for organizing logic based on what menu it is on
class Menus:
    START = "start"
...
class FlashTracker:
    ...
    def main_loop(self):
        current_menu = Menus.START
        line_index = 0
        last_line_index = line_index
        while True:
            if (current_menu == Menus.START):
                print("\nAlex's flash tracker v1.0\n------------------\n")
                print("1. Join a already existing lobby")
                print("2. Create a new lobby")
                choice = input("> ")

                if (int(choice) == 1):
                    current_menu = Menus.JOIN_LOBBY
                elif (int(choice) == 2):
                    current_menu = Menus.CREATE_LOBBY

            elif (current_menu == Menus.CREATE_LOBBY):
                print("\nPlease enter name of the lobby (one word only. spaces will be converted into underscores)")
                lobby_name = input("> ").replace(" ", "_")

                creation_attempt = self.server.create_lobby(lobby_name)
                if (creation_attempt == True):
                    os.system("cls")
                    current_menu = Menus.MAIN
                else:
                    print("That lobby name is already in use!")

            elif (current_menu == Menus.MAIN):
                print("In loby\n--------\n")
